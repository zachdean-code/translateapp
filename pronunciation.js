window.Pronunciation = (() => {
  const normalize = window.AppHelpers?.normalize || ((text) => (text || "").toLowerCase());
  ...
  function spanishPronunciationForEnglishReader(text) {
    return sanitizeForSpeech(text)
      ...
      .filter(Boolean)
      .map((word) => {
        const clean = normalize(word).replace(/[^a-zñ]/g, "");
        if (!clean) return "";
  ...
  function buildPronunciation(text, sourceLanguage, targetLanguage) {
    ...
    if (source.includes("spanish") && target.includes("english")) {
      return normalizePronunciationStyle(englishPronunciationForSpanishReader(text));
    }

    if (source.includes("english") && target.includes("spanish")) {
      return normalizePronunciationStyle(spanishPronunciationForEnglishReader(text));
    }
  ...
  return {
    sanitizeForSpeech,
    normalizePronunciationStyle,
    hasPronunciationSupport,
    buildPronunciation
  };
})();
